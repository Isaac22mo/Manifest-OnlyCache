...
function fetchCocktail() {
    ...
    fetch(url)
        .then(response => {
            if (!response.ok) {
                // Esta línea maneja errores HTTP 4xx/5xx, no fallos de red (que los maneja el SW)
        ...
        .catch(error => {
            // Este catch solo debe atrapar errores del lado del cliente (ej. fallo en el parseo JSON)
